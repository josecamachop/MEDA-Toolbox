function [Q,lvso,gammaso,press] = dcrossval_gpls1(x,y,lvs,gammas,alpha,blocks_r,prepx,prepy,opt,mtype)

% Row-wise k-fold (rkf) double cross-validation for square-prediction-errors computing in GPLS1.
%
% Q = dcrossval_gpls1(x,y) % minimum call
% [Q,lvso,gammaso,press] = dcrossval_gpls(x,y,lvs,gammas,alpha,blocks_r,prepx,prepy,opt,mtype) % complete call
%
%
% INPUTS:
%
...
% lvs = 0:10;
% gammas = 0:0.1:1;
% [Q,lvso,gammaso] = dcrossval_gpls1(X,Y,lvs,gammas,1,7);
% [Qb,lvsob,gammasob] = dcrossval_gpls1(X,Y,lvs,gammas,0.5,7);
%
%
...
if nargin < 10 || isempty(mtype), mtype = 3; end;
...
assert (isequal(size(mtype), [1 1]), 'Dimension Error: 10th argument must be 1-by-1. Type ''help %s'' for more info.', routine(1).name);
...
assert (isempty(find(mtype~=1 & mtype~=2 & mtype~=3 & mtype~=4)), 'Value Error: 10th argument must contain an integer from 1 to 4. Type ''help %s'' for more info.', routine(1).name);
...
for i=1:blocks_r,
    ...
    [cumpress,kk,nze] =  crossval_gpls1(rest,rest_y,lvs,gammas,blocks_r-1,prepx,prepy,0,mtype);
    ...
    beta = gpls1(ccs,ccs_y,1:lvso(i),gammaso(i),mtype);
    srec = vcs*beta;
