...
for j = 1 : n_perm
    ...
    for i = 1 : n_interactions
        ...
        F_interactions(1 + j,i,:) = squeeze(SSQ_interactions(1 + j,i,:)/df_int(i))./(SSQ_residualsp/Rdf)';
    end
...
for factor = 1 : n_factors
    ...
    for j = 1 : n_perm
        ts_factors(1 + j,factor,:) = sort(ts_factors(1 + j,factor,:),'descend');
    end
end
for interaction = 1 : n_interactions
    [~,ord_interactions(interaction,:)] = sort(ts_interactions(1,interaction,:),'descend');
    for j = 1 : n_perm
        ts_interactions(1 + j,interaction,:) = sort(ts_interactions(1 + j,interaction,:),'descend');
    end
end
...
for factor = 1 : n_factors
    for var = 1 : M
        p_factor(factor,ord_factors(factor,var)) = (size(find( sum(ts_factors(2:end,factor,1:var),3) ...
            >= sum(ts_factors(1, factor,ord_factors(factor,1:var)))) ,1) + 1)/(n_perm+1);
    end
end
for interaction = 1 : n_interactions
    for var = 1 : M
        p_interaction(interaction,ord_interactions(interaction,var)) = (size(find( sum(ts_interactions(2:end,interaction,1:var),3) ...
            >= sum(ts_interactions(1, interaction, ord_interactions(interaction,1:var)))), 1) + 1)/(n_perm+1);
    end
end
...
if n_interactions
    SSQ = sum([SSQ_inter' squeeze(SSQ_factors(1,:,:)) squeeze(SSQ_interactions(1,:,:)) SSQ_residuals' SSQ_X'],1);
else
    SSQ = sum([SSQ_inter' squeeze(SSQ_factors(1,:,:)) SSQ_residuals' SSQ_X'],1);
end
