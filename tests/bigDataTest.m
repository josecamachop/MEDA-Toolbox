classdef bigDataTest < matlab.unittest.TestCase
    ...
    methods(Test)
        ...
        function testvarLpca(testCase)
            ...
            model.lvs = 0:10;
            varLpca(model, 'Plot', false);
            varLpca(model, 'Plot', true);
            close all
        end
        ...
        function testvarLpls(testCase)
            ...
            model = iniLmodel(X,Y);
            model.lvs = 0:10;
            varLpls(model, 'Plot', false);
            varLpls(model, 'Plot', true);
            close all
        end
        ...
        function testscoresLpls(testCase)
        nobs = 100;
        nvars = 10;
        X = simuleMV(nobs,nvars,'LevelCorr',8);
        Y = 0.1*randn(nobs,2) + X(:,1:2);
        ...
        Lmodel.lvs = 1:2;
        [T,TT] = scoresLpls(Lmodel,'Test',test);
        close all
        end
