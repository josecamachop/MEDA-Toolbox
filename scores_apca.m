function T = scores_apca(paranovao, alpha, labels)
...
%
% T = scores_apca(paranovao)   % minimum call
% T = scores_apca(paranovao, alpha, labels)   % complete call
%
%
% INPUTS:
%
...
%
% T = scores_apca(paranovao,0.05,{'Factor 1','Factor 2'});
%
%
...
for i=1:length(ind)
    ...
    T{ind(i)} = scores_pca(xf+e,1:rank(xf),[],0,100,[],paranovao.design(:,ind(i)));
    if ~isempty(labels),
        title(labels{ind(i)})
    end
end
