function  [beta,W,P,Q,R,bel] = gpls1(xcs,ycs,lvs,gamma,mtype)

% Group-wise Partial Least Squares for prediction, algorithm 1. This routine
...
%
% beta = gpls1(xcs,ycs)     % minimum call
% [beta,W,P,Q,R,bel] = gpls1(xcs,ycs,lvs,gamma,mtype)    % complete call
%
%
% INPUTS:
%
...
% [beta,W,P,Q,R,bel] = gpls1(Xcs,Ycs,lvs);
% 
% for i=lvs,
...
if nargin < 5 || isempty(mtype), mtype = 3; end;
...
assert (isequal(size(mtype), [1 1]), 'Dimension Error: 5th argument must be 1-by-1. Type ''help %s'' for more info.', routine(1).name);
...
assert (isempty(find(mtype~=1 & mtype~=2 & mtype~=3 & mtype~=4)), 'Value Error: 5th argument must contain an integer from 1 to 4. Type ''help %s'' for more info.', routine(1).name);
