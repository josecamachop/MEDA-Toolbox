function varargout = vASCA(varargin)

% GUI for (variable-wise) ANOVA Simultaneous Component Analysis (vASCA) 
%
% This M-file include routines from the MEDA Toolbox: 
% parglm.m, parglmVS.m, asca.m, vasca.m, loadings.m, meda.m, omeda.m, 
% scores.m, mspc.m and leverages.m 
%
% vASCA % minimum call
% vASCA(x,y,lvs,prepX,prepY) % complete call
%
%
% INPUTS:
%
...
% vASCA M-file for vASCA.fig
%      vASCA, by itself, creates a new vASCA or raises the existing
%      singleton*.
%
%      H = vASCA returns the handle to a new vASCA or the handle to
%      the existing singleton*.
%
%      vASCA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in vASCA.M with the given input arguments.
%
%      vASCA('Property','Value',...) creates a new vASCA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vASCA_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vASCA_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vASCA

% Last Modified by GUIDE v2.5 26-Nov-2024 06:44:52
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @vASCA_OpeningFcn, ...
    'gui_OutputFcn',  @vASCA_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before vASCA is made visible.
function vASCA_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vASCA (see VARARGIN)

% Choose default command line output for vASCA
   ...
   % set(handles.refreshbutton,'Enable','off');
    ...
    if length(varargin) > 2    
        ...
        if length(varargin) > 3 & ~isempty(varargin{4})
            ...
            generalPopup_Callback(handles.varselPopup, eventdata, handles)
            handles = guidata(handles.varselPopup);
...
function varargout = vASCA_OutputFcn(hObject, eventdata, handles)
...
generalPlot = getCurrentPopupString(handles.varselPopup);
model = getCurrentPopupString(handles.modelPopup);
coding = getCurrentPopupString(handles.codingPopup);
nested = getCurrentPopupString(handles.nestedPopup);
switch generalPlot
    case 'ASCA'
        [T,struct] = parglm(X, F, 'Model',model,'Coding',coding,'Nested',nested);
        model = asca(struct);
    case 'VASCA'
        [T,struct] = parglmVS(X, F, 'Model',model,'Coding',coding,'Nested',nested);
        model = vasca(struct);
    otherwise
        disp('No case detected')
end
...
if isempty(handles.data.label) && isempty(handles.data.classes)
    [T,TT]=scoresPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',[LV1 LV2],'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1);
else
    if ~isempty(handles.data.label) && isempty(handles.data.classes)
        [T,TT]=scoresPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',[LV1 LV2],'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1,'ObsLabel',handles.data.label);
    else
        if isempty(handles.data.label) && ~isempty(handles.data.classes)
            [T,TT]=scoresPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',[LV1 LV2],'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1,'ObsClass',handles.data.classes);
        else
            [T,TT]=scoresPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',[LV1 LV2],'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1,'ObsLabel',handles.data.label,'ObsClass',handles.data.classes);
        end
    end
end
fig=gcf;
...
if ~isempty(handles.data.weightDummy{1,ID})
    ...
    omedaPls(handles.data.data_matrixX,handles.data.data_matrixY,[min(LV1,LV2) max(LV1,LV2)],handles.data.data_matrixX,handles.data.weightDummy{1,ID}','PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1,'VarsLabel',handles.data.label_LP,'VarsClass',handles.data.classes_LP);
else
    omedaPls(handles.data.data_matrixX,handles.data.data_matrixY,[min(LV1,LV2) max(LV1,LV2)],handles.data.data_matrixX,handles.data.dummy{1,ID}','PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1,'VarsLabel',handles.data.label_LP,'VarsClass',handles.data.classes_LP);
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Dst,Qst,Dstt,Qstt,UCLd,UCLq] = mspcPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',min(handles.data.LVs):max(handles.data.LVs),'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',0,'ObsLabel',handles.data.label,'ObsClass',handles.data.classes);
...
[meda_map,meda_dis]=medaPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',lvs,'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Threshold',handles.data.thres,'Option',handles.data.opt,'VarsLabel',handles.data.label_LP);
guidata(hObject,handles);
...
[meda_map,meda_dis]=medaPls(handles.data.data_matrixX,handles.data.data_matrixY,[min(handles.data.LV1_LP,handles.data.LV2_LP) max(handles.data.LV1_LP,handles.data.LV2_LP)],handles.data.prepX,handles.data.prepY,handles.data.thres,handles.data.opt,handles.data.label_LP,vector_vars);
guidata(hObject,handles);
...
E=leveragesPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',PCs,'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1,'VarsLabel',handles.data.label_LP,'ObsClass',handles.data.classes_LP);
ylabel('Residuals')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
E=leveragesPls(handles.data.data_matrixX,handles.data.data_matrixY,'LVs',handles.data.LVs,'PreprocessingX',handles.data.prepX,'PreprocessingY',handles.data.prepY,'Option',1,'VarsLabel',handles.data.label_LP,'ObsClass',handles.data.classes_LP);
...
% --- Executes on selection change in varselPopup.
function varselPopup_Callback(hObject, eventdata, handles)
% hObject    handle to varselPopup (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns varselPopup contents as cell array
%        contents{get(hObject,'Value')} returns selected item from varselPopup
...
generalSelection = getCurrentPopupString(handles.varselPopup);
...
% --- Executes when uipanelGen is resized.
function uipanelGen_SizeChangedFcn(hObject, eventdata, handles)
% hObject    handle to uipanelGen (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on selection change in codingPopup.
function codingPopup_Callback(hObject, eventdata, handles)
% hObject    handle to codingPopup (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns codingPopup contents as cell array
%        contents{get(hObject,'Value')} returns selected item from codingPopup


% --- Executes during object creation, after setting all properties.
function codingPopup_CreateFcn(hObject, eventdata, handles)
% hObject    handle to codingPopup (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in nestedPopUp.
function nestedPopUp_Callback(hObject, eventdata, handles)
% hObject    handle to nestedPopUp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns nestedPopUp contents as cell array
%        contents{get(hObject,'Value')} returns selected item from nestedPopUp


% --- Executes during object creation, after setting all properties.
function nestedPopUp_CreateFcn(hObject, eventdata, handles)
% hObject    handle to nestedPopUp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in factorPopUp.
function factorPopUp_Callback(hObject, eventdata, handles)
% hObject    handle to factorPopUp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns factorPopUp contents as cell array
%        contents{get(hObject,'Value')} returns selected item from factorPopUp


% --- Executes during object creation, after setting all properties.
function factorPopUp_CreateFcn(hObject, eventdata, handles)
% hObject    handle to factorPopUp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function varselPopup_CreateFcn(hObject, eventdata, handles)
% hObject    handle to varselPopup (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
