function [T, parglmo] = parglmVM(X, F, varargin)
...
%
% T = parglmVM(X, F)   % minimum call
%
% See also: parglm, parglmVS, parglmMC, vasca, createDesign
%
%
% INPUTS:
%
...
% rng(S);
% [TVS, parglmoVS] = parglmVM(X, class); % With variable selection
% 
% h = figure; hold on
...
% [T, parglmo] = parglm(X,class); % No variables selection 
% rng(S);
% [TVS, parglmoVS] = parglmVM(X, class); % With multivariate variable selection
% 
% h = figure; hold on
% plot([1 nVars],-log10([parglmo.p parglmo.p]),'b-.')
% plot(-log10(parglmoVS.p(parglmoVS.ordFactors)),'g-o')
...
% 
% [tableVs, parglmoVs] = parglmVM(X, F, 'Model',{[1 2]}, 'Random', [1 1]);
% tableVs
% 
